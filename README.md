...
│   └── capacity.json        运行中学到的最大安全 QPS，实时原子保存
├── probe/
│   └── probe.py
├── proxy/
...
| `GET /routes` | 每条路由的当前 QPS、持久化的最大安全 QPS、限流时观测到的 `others` QPS、按五个「面」拆开的 QPS、当前权重、429 次数、降权状态、钉住的会话 |
...
**源** —— 一个 endpoint 一张卡。这个资源上哪些部署在被用、当前 QPS 和最大安全 QPS。
一个 endpoint 是一份钱、一个爆炸半径，所以按它分组。

**模型** —— 一个模型一张卡。显示活跃 session 数量及类型、可用源、当前 QPS 和最大安全
QPS。`codex` 表示普通 Codex session，`subagent` 表示输入中带 `agent_message` 的子 agent
session。
...
只看问题 → 只看「他人流量观测变了」。最右边一列固定是**这件事改变了什么**：
...
条形的分母是这条路由历史上成功承载过的最大 QPS：

- **current**（鼠尾草绿）：当前一秒窗口内的 QPS。
  五个字符是同一个数的五个切片，**同色不同字符**：`█` chat、`▓` chat 流式、
  `▒` responses、`▚` responses 流式、`▞` 图像。它们抢的是同一个上限，所以是一个
  颜色；给五种颜色会读成五条互不相干的条恰好挨在一起，那是错的心智模型。
- **░ others**（灰褐）：当当前 QPS 低于历史最大值却触发限流时，两者的差值。
- **· available**（暗）：最大安全 QPS 中尚未被 current 和 others 占用的部分。

**空条 `────` 加 `—` 表示还没有成功请求可用于建立安全 QPS。** 每个成功请求都会用它发出
时的一秒窗口 QPS 更新最大值；最大值只增不减，并立即原子写入 `runtime/capacity.json`。
