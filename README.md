...
模型页使用与源页一致的列数和卡片宽度。
当单列将超过 93 个字符格时，提前增加列数，均分窗口宽度；
当所有卡片已各占一列且均达到宽度上限时，右侧留白。
摘要与首条明细之间、相邻明细之间、末条明细与下边框之间各留一行空白。
框内每行与左右边框各留两格空白。

**模型**：一个模型一张卡，按源展示当前用量。卡片摘要统一显示 `MAX RPM: n`，
数值单独着色并位于卡片右上角；源/deployment 数量、请求方式列表及版本日期隐藏。
卡片标题居中显示名称；标题下同一行显示 pinned 总数、旁边的 `RPM:cur/avail`、隐藏旧模型数及右对齐的 `MAX RPM`。
模型卡片首行显示 `chat/resp route prob(%)`，明细首列按接口分别显示概率，如 `12/35`。
概率取整数百分比，每侧占两字符；个位数左侧补空格，100% 显示为 `1ꝏ`（U+A74F），
接口不支持或旧快照缺少分接口概率时显示 `--`。各接口的流式与非流式共用概率。
卡片较窄时优先保留概率说明，省略 pinned 和 RPM 用量说明；必要时将 `MAX RPM` 简写为 `RPM`。
条目的 pinned 数量使用独立的右对齐列，颜色比 RPM 更浅。
数量为零或缺失时，摘要和每条 deployment 明细均显示 `·` 占位。
完整的 `n pinned` 只在标题下第一行显示，明细列只显示数字或 `·`。
用量与条形同一行显示为 `cur/avail ───── n`：左侧绿色数字为当前 RPM/可用 RPM，右侧为 others；最大 RPM 仍在最右列。
可用 RPM 使用路由报告中的 `available_rpm`；旧快照按容量减去当前和外部用量计算，最低为零，容量未知时显示 `—`。
